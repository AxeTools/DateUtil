...
class DateTimeUtil {
     ...
     */
    public static function isHoliday(array $holidays, \DateTimeInterface $referenceDatetime, $observation_options = self::HOLIDAY_OBSERVED_NONE) {
        foreach ($holidays as $holiday) {
            ...
            if ($referenceDatetime->format('Ymd') == $holiday->getDatetime()->format('Ymd')) return true;
        }
        return false;
    }
     ...
     * @see Week
     */
    public static function createRelativeDateTime($month, $dayOfWeek, $weekOfMonth, $year = null) {
        ...
        $date = DateTime::createFromFormat('Y-m-d H:i', $year . "-" . $month . "-01 00:00");
     ...
     */
    public static function createAbsoluteDateTime($month = null, $day = null, $year = null) {
        $year = (null === $year) ? date('Y') : $year;
        $month = (null === $month) ? date('m') : $month;
        ...
        $date = DateTime::createFromFormat('Y-m-d H:i', $year . "-" . $month . "-" . $day . " 00:00");
        if (false === $date) throw new InvalidArgumentException('Invalid date format');
        return $date;
    }
     ...
     * @return bool
     */
    public static function isBetween(\DateTimeInterface $start, \DateTimeInterface $end, \DateTimeInterface $referenceDatetime, $inclusiveOptions = DateTimeUtil::IS_BETWEEN_INCLUSIVE) {
